...
| `Flow Address`              | `[OPCODE: 5][RESERVED: 3][ADDRESS: 24]`                |
...
Всего 4 этапа исполнения инструкций:
...
2) `Address Fetch` (выборка адреса) - для адресных команд, команд с операндом и команд перехода выбирается адрес
3) `Operand Fetch` (выборка операнда) - выполняется только для команд с операндом
