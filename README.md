...
| 6  | `not`      | `~AC         -> AC`                 | побитовое логическое "НЕ"                               |
| 7  | `ld A`     | `MEM[A] -> AC`                      | загрузка значения в аккумулятор по адресу               |
| 8  | `st A`     | `AC -> MEM[A]`                      | сохранение значения из аккумулятора по адресу           |
| 9  | `put`      | `AC -> IO`                          | вывод значения из аккумулятора                          |
| 10 | `get`      | `IO -> AC`                          | ввод значения в аккумулятор                             |
| 11 | `push`     | `SP - 1 -> SP`                      | подъем указателя стека                                  |
| 12 | `pop`      | `SP + 1 -> SP`                      | понижение указателя стека                               |
| 13 | `jmp A`    | `A -> IP`                           | безусловный переход                                     |
| 14 | `jz A`     | `A -> IP, if AC == 0`               | переход, если в аккумуляторе `0`                        |
| 15 | `call A`   | `IP -> STACK, FP -> STACK, A -> IP` | вызов функции                                           |
| 16 | `ret`      | `STACK -> FP, STACK -> IP`          | возврат из функции                                      |
| 17 | `ispos`    | `(AC > 0) -> AC`                    | проверка, что в аккумуляторе строго положительное число |
| 18 | `isneg`    | `(AC < 0) -> AC`                    | проверка, что в аккумуляторе строго отрицательное число |
| 19 | `iszero`   | `(AC == 0) -> AC`                   | проверка, что в аккумуляторе `0`                        |
| 20 | `nop`      |                                     | бездействие                                             |
| 21 | `halt`     |                                     | остановка исполнения                                    |
