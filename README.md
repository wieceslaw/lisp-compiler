...
- описать, как работает FLAGS (коммутация АЛУ)
...
### Литералы
- строковые - сама строка помещается в статическую память, закодированную в pascal-style, в месте объявления заменяется на адрес расположения строки;
...
`Caller`:
- на стек помещаются аргументы функции после вычисления `k` выражений (число аргументов всегда **равно** числу параметров, принимаемых функцией)
- происходит вызов инструкции `call A`:
  - на стек помещается адрес возврата (текущий `IP`)
  - происходит переход на инструкции функции: `IP = A`
  - сохраняется текущий указатель стека `FP`
  - устанавливается новый указатель фрейма функции: `FP = SP`
